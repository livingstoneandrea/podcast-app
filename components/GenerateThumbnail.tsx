...
const GenerateThumbnail = ({setImage, setImageStorageId, image, imagePrompt, setImagePrompt}: GenerateThumbnailProps) => {
  ...
  const generateImage = async () => {
    try {
      ...
      console.log(error)
      toast({
        title: "Error in generating thumbnail",
        variant: "destructive",
      })
    }
    ...
    <>
      ...
      {isAiThumbnail ? (
        <div className="flex flex-col gap-5">
           <div className="mt-5 flex flex-col gap-2.5">
            ...
            <Textarea className="input-class font-light focus-visible:ring-offset-orange-1"
                placeholder="Provide  text to generate thumbanil"
                rows={5}
                value={imagePrompt}
        ...
        <div className="image_div" onClick={()=> imageRef?.current?.click()}>
          <Input type="file" className="hidden" ref = {imageRef} accept="image/*" onChange={(e) => uploadImage(e)} />
          {!isImageLoading ? (
           ...
           <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-12 font-normal text-gray-1">Supported formats: SVG, JPG, JPEG, PNG or GIF (max. 1080*1080px)</p>
          </div>
        </div>
      )}
      {image && (
